    ...
    <el-table v-loading="loading" :data="configList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="启用" align="center" prop="status">
        <template slot-scope="{ row }">
          <el-switch v-model="row.status" @change="onStatusChange(row)"></el-switch>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px" style="padding: 0 30px">
        <el-form-item label="HostName" prop="hostname">
          <el-input v-model="form.hostname" placeholder="请输入域名" />
        </el-form-item>
        <el-form-item label="IP" prop="ip">
          <el-input v-model="form.ip" placeholder="请输入IP" />
        </el-form-item>
        <el-form-item label="端口" prop="port">
          <el-input v-model="form.port" placeholder="请输入$端口" />
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除Veeam服务器编号为"' + ids + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('veeam/serverConfig/export', {
        ...this.queryParams
